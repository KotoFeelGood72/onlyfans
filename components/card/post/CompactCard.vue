<template>
  <div class="card" :class="{ 'empty-card': post.isLock }">
    <img :src="post.img" v-if="!post.isLock" />
    <div class="card__txt" v-if="!post.img && !post.isLock">{{ post.isText }}</div>
    <ul class="card__info" :class="{ isImg: post.img }">
      <li v-if="post.countImg">
        <Icon name="solar:gallery-bold" size="12" />{{ post.countImg }}
      </li>
      <li v-if="post.isText">
        <Icon name="ph:text-a-underline" size="12" />
      </li>
      <li v-if="post.isQuiz">
        <Icon name="ph:chart-bar-horizontal-light" size="12" />
      </li>
    </ul>
    <div v-if="post.isLock" class="lock__icon"><Icon size="60" name="prime:lock" /></div>
  </div>
</template>
